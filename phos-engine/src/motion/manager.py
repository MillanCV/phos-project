...
class MotionSessionRepository(Protocol):
    def save(self, session: MotionSession) -> MotionSession: ...
...
class MotionManager:
    def __init__(
        ...
        repository: MotionSessionRepository,
    ) -> None:
    ...
    def start_session(self, profile_name: str, commands: list[str], timeout_seconds: int = 300) -> MotionSession:
        if not commands:
            raise ValidationError("commands cannot be empty")
        if timeout_seconds <= 0:
            raise ValidationError("timeout_seconds must be > 0")

        session = MotionSession.new(
    ...
    def stop_session(self, session_id: str) -> MotionSession:
    ...
    def get_session(self, session_id: str) -> MotionSession:
        session = self._repository.get(session_id)
        if not session:
            raise NotFoundError(f"motion session {session_id} not found")
        if session.run_id:
